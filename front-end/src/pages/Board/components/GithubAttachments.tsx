...
export default function GithubAttachments({
  boardId,
  cardId,
  taskId,
  isOwner,
    ...
    <div className="mb-4">
      ...
      >
        <div>
          ...
          {githubInfo && (
            <div
              className="github-info-results pe-2"
              style={{ maxHeight: "350px", overflowY: "auto" }}
            >
              <div className="small fw-bold text-secondary mb-2 mt-1">
                Pull Requests
              </div>
              {githubInfo.pulls.length === 0 && (
                <div className="small text-white-50">No PRs found</div>
              )}
              {githubInfo.pulls.map((pr) => (
                <div
                  key={`pr-${pr.pullNumber}`}
                  className="d-flex justify-content-between align-items-center mb-2 text-white small border-bottom border-secondary pb-1"
                >
                  ...
                  <Button
                    ...
                    className="py-0 px-2"
                    style={{ fontSize: "12px" }}
                    onClick={() => handleAttach("pull_request", pr.pullNumber)}
                  >
                    Attach
                  </Button>
                </div>
              ...
              <div className="small fw-bold text-secondary mt-3 mb-2">
                Issues
              </div>
              {githubInfo.issues.length === 0 && (
                <div className="small text-white-50">No Issues found</div>
              )}
              {githubInfo.issues.map((issue) => (
                ...
                >
                  ...
                  <Button
                    ...
                    className="py-0 px-2"
                    style={{ fontSize: "12px" }}
                    onClick={() => handleAttach("issue", issue.issueNumber)}
                  >
                    Attach
                  </Button>
                </div>
              ...
              {githubInfo.commits.length === 0 && (
                <div className="small text-white-50">No Commits found</div>
              )}
              {githubInfo.commits.map((commit) => (
                ...
                >
                  ...
                  <Button
                    ...
                    style={{ fontSize: "12px" }}
                    onClick={() =>
                      handleAttach("commit", undefined, commit.sha)
                    }
                  >
                    Attach
                  </Button>
      ...
      {attachments.length > 0 ? (
        <div className="d-flex flex-column gap-2">
          {attachments.map((att) => (
            <div
              key={att._id || att.attachmentId}
              className="d-flex align-items-center justify-content-between github-attachment-item"
            >
              <div className="d-flex align-items-center gap-2">
                ...
                {att.type === "commit" && (
                  <i className="bi bi-code-commit text-info"></i>
                )}
